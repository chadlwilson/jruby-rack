...
describe "integration" do
  ...
  describe 'rails 7.1', :lib => :rails71 do
    ...
    before(:all) { copy_gemfile("rails71") }

    def base_path; "file://#{STUB_DIR}/rails71" end
    ...
    context "initialized" do
      ...
      it "booted with a servlet logger" do
        ...
        should_eval_as_not_nil "defined?(Rails)"
        should_eval_as_not_nil "Rails.logger"
        should_eval_as_eql_to "Rails.logger.class", ActiveSupport::BroadcastLogger
        should_eval_as_eql_to "Rails.logger.broadcasts[0].instance_variable_get(:'@logdev').dev.class.name",
                              'JRuby::Rack::ServletLog'
        should_eval_as_eql_to "Rails.logger.level", Logger::INFO
      end
      ...
      it "sets up public_path (as for a war)" do
        @runtime = @rack_factory.getApplication.getRuntime
        should_eval_as_eql_to "Rails.public_path.to_s", "#{STUB_DIR}/rails71"
      end
